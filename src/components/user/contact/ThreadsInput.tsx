...
const formSchema = z.object({
  threadsUrl: z.string().min(1, {
    message: "La url de Threads es requerida",
  }),
});

export const ThreadsInput = ({ id, initialData }: ThreadsInputProps) => {
  const { toast } = useToast();
  const [isEditing, setisEditing] = useState(false);
  const [currentData, setCurrentData] = useState(initialData);

  const threadsUrl = currentData?.threadsUrl || "";
        ...
        <div className="pt-6 sm:flex">
          ...
          <dd className="mt-1 flex justify-between items-center gap-x-6 sm:mt-0 sm:flex-auto">
            {isEditing ? (
                      ...
                      <FormItem className="bg-white w-full">
                        <FormControl>
                          <Input
                            ...
                            autoFocus
                            placeholder="https://www.threads.net/@holistia"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
              ...
              <div className="text-gray-900">{threadsUrl}</div>
            )}
