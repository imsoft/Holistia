...
interface RestaurantGalleryManagerProps {
  restaurantId: string;
...
export function RestaurantGalleryManager({
  restaurantId,
  currentImages,
  onImagesUpdate,
  maxImages = 4,
  maxSizeMB = 2
}: RestaurantGalleryManagerProps) {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Subir imagen a Supabase Storage (usando bucket restaurants)
    ...
    <div className="space-y-4">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Selecciona una imagen para agregar a la galería del restaurante.
              </DialogDescription>
            </DialogHeader>
            ...
            <CardContent className="p-2">
              <div className="relative aspect-square rounded-lg overflow-hidden">
                ...
                <Button
                  variant="destructive"
                  size="sm"
                  className="absolute top-2 right-2 opacity-80 hover:opacity-100 transition-opacity shadow-lg"
      ...
      {currentImages.length === 0 && (
        <div className="text-center py-8 border-2 border-dashed border-muted-foreground/25 rounded-lg">
          ...
          <p className="text-sm text-muted-foreground">
            Agrega hasta {maxImages} imágenes para mostrar el restaurante
          </p>
        </div>
      )}
    </div>
