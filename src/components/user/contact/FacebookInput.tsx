...
const formSchema = z.object({
  facebookUrl: z.string().min(1, {
    message: "La url de Facebook es requerida",
  }),
});

export const FacebookInput = ({ id, initialData }: FacebookInputProps) => {
  const { toast } = useToast();
  const [isEditing, setisEditing] = useState(false);
  const [currentData, setCurrentData] = useState(initialData);

  const facebookUrl = currentData?.facebookUrl || "";
        ...
        <div className="pt-6 sm:flex">
          ...
          <dd className="mt-1 flex justify-between items-center gap-x-6 sm:mt-0 sm:flex-auto">
            {isEditing ? (
                      ...
                      <FormItem className="bg-white w-full">
                        <FormControl>
                          <Input
                            ...
                            autoFocus
                            placeholder="https://www.facebook.com/holistia"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
              ...
              <div className="text-gray-900">{facebookUrl}</div>
            )}
