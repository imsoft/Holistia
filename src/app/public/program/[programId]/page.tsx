...
export default function PublicProgramPage({
    ...
    try {
      ...
      if (!professionalData && productData.professional_id) {
        const { data: professionalManual, error: professionalError } = await supabase
          ...
          .select("id, first_name, last_name, profile_photo, is_verified, profession, specializations")
  ...
  const handlePurchase = async () => {
    if (!currentUserId) {
      ...
      router.push(`/login?redirect=/public/program/${programId}`);
      return;
    }
                  ...
                  <Card className="hover:shadow-md transition-shadow">
                    <CardContent className="p-6">
                      <div className="space-y-4">
                        <div className="flex items-start gap-4">
                          <Link
                            href={`/public/professional/${product.professional_id}`}
