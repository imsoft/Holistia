...
const MapboxMap = dynamic(() => import("@/components/ui/mapbox-map"), {
  ssr: false,
...
export interface ProfessionalProfileClientProps {
...
export function ProfessionalProfileClient({
  ...
  initialIsFavorite,
  digitalProducts: initialDigitalProducts,
  ...
  const [professional] = useState<Professional | null>(initialProfessional);
  const [currentUser] = useState<CurrentUser | null>(initialCurrentUser);
  ...
  const [appointmentForm, setAppointmentForm] = useState({
    ...
    phone: initialCurrentUser?.phone || "",
    notes: "",
  });
  ...
  const [digitalProducts] = useState<any[]>(initialDigitalProducts);
  const [challenges] = useState<any[]>(initialChallenges);
  ...
  useUserStoreInit();
  const resolvedProfessionalId = initialProfessional?.id || null;
  ...
  // Loading state handled by server component + loading.tsx
              ...
              <div className="space-y-2 sm:space-y-3">
                {!isAuthenticated ? (
                  <>
                    <Button
                      type="button"
                      onClick={() => router.push(`/signup?redirect=${encodeURIComponent(`/explore/professional/${slug}`)}`)}
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => router.push(`/login?redirect=${encodeURIComponent(`/explore/professional/${slug}`)}`)}
