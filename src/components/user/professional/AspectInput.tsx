...
const formSchema = z.object({
  aspect: z.string().min(1, {
    message: "El Aspecto es requerido",
  }),
});

export const AspectInput = ({ id, initialData }: AspectInputProps) => {
  const { toast } = useToast();
  const [isEditing, setisEditing] = useState(false);
  const [currentData, setCurrentData] = useState(initialData);

  const aspect = currentData?.aspect || "";
        ...
        <div className="pt-6 sm:flex">
          ...
          <dd className="mt-1 flex justify-between items-center gap-x-6 sm:mt-0 sm:flex-auto">
            {isEditing ? (
              ...
              <div className="text-gray-900">{aspect}</div>
            )}
