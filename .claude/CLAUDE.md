...
- Framework: Next.js 16.1.1 (App Router con Turbopack)
- React: 19.2.3
...
- Las rutas dinámicas usan `[id]`, `[slug]`, etc.
- Los grupos de rutas usan paréntesis: `(dashboard)`, `(auth)`

### Componentes
- `/src/components/ui/` - Componentes reutilizables de UI (shadcn/ui)
- `/src/components/appointments/` - Componentes específicos de citas
- `/src/components/reviews/` - Componentes de reseñas
...
- `/database/scripts/` - Scripts SQL de utilidad y verificación
...
- Definir interfaces en `/src/types/` o inline si son muy específicas
- Usar tipos union para estados: `'pending' | 'confirmed' | 'cancelled'`
- Campos opcionales con `?` o `| null` según el caso
...
- Componentes UI en `/src/components/ui/` deben seguir el patrón de shadcn/ui
- Responsive: mobile-first con breakpoints de Tailwind

### Supabase y Base de Datos
- SIEMPRE usar RLS (Row Level Security) - nunca deshabilitar sin justificación
- Verificar autenticación con `supabase.auth.getUser()` antes de operaciones
...
- Retornar códigos HTTP apropiados (200, 400, 401, 404, 500)
...
- Variables de entorno públicas deben empezar con `NEXT_PUBLIC_`
...
- Mostrar errores de validación de forma clara al usuario
...
- Manejar errores específicos de Supabase (ej: PGRST116 para "no rows found")

### Imágenes y Archivos
- Usar Next.js Image component para optimización
- Storage buckets en Supabase: `profiles`, `restaurants`, `holistic-centers`, `blog`
- Rutas de storage: `/<entity-id>/imagen.{ext}`
- Validar tipos de archivo y tamaño antes de subir

### Variables de Entorno
- Públicas (cliente): `NEXT_PUBLIC_*`
- Privadas (servidor): `SUPABASE_SERVICE_ROLE_KEY`, `STRIPE_SECRET_KEY`, `RESEND_API_KEY`
- Archivo `.env.local` NO se sube a git (está en .gitignore)

### Email
- Templates HTML en `/database/email-templates/`
- Usar Resend API (`resend` package)
- Función helper: `sendEmail()` con templates
...
- Layouts específicos en `/app/(dashboard)/(admin|professional|patient)/`
- Cada dashboard tiene su propio sidebar y navegación
...
- Integración con Stripe para pagos de eventos
- Sistema de confirmación con códigos
...
- Componente: `StarRating` para visualización

### Blog
- Tabla: `blog_posts`
- Storage bucket: `blog` para imágenes
...
- Caché apropiado en Supabase queries cuando sea posible
...
### Testing y Debugging
- Usar console.log con emojis para debugging: 🔍, ✅, ❌, ⚠️
- Verificar errores en Supabase Dashboard
- Revisar logs de Vercel en producción
- Scripts de verificación en `/database/scripts/`

### Git y Commits
- Commits descriptivos en español
- Separar cambios lógicos en commits diferentes
- No commitear `.env.local` o archivos sensibles
- En los commits hacer dos tipos de explicaciones una técnica y otra para que cualquier persona (una persona no tecnica) lo pueda entender
...
⚠️ **NUNCA:**
- Deshabilitar RLS sin justificación técnica
- Exponer service role keys en el cliente
- Hacer commits de archivos `.env*`
- Usar `any` en TypeScript sin comentar por qué
...
- Documentar código complejo o no obvio
