      ...
      try {
        ...
        if (professionalApp) {
          ...
          if (appointmentsData && appointmentsData.length > 0) {
            ...
            const { data: patientsData, error: patientsError } = await supabase.auth.admin.listUsers();
            
            if (!patientsError && patientsData) {
              const formattedAppointments: Appointment[] = appointmentsData.map(apt => {
                const patient = patientsData.users.find(u => u.id === apt.patient_id);
                ...
                return {
                  ...
                  patient: {
                    ...
                    email: patient?.email || '',
                    phone: patient?.user_metadata?.phone || patient?.phone || '',
                  },
