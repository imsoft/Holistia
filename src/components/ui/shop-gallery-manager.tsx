...
export function ShopGalleryManager({
  shopId,
  currentImages,
  onImagesUpdate,
  maxImages = 4,
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Subir imagen a Supabase Storage (usando bucket shops)
    ...
    <div className="space-y-4">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Selecciona una imagen para agregar a la galería del comercio.
              </DialogDescription>
            </DialogHeader>
      ...
      {currentImages.length === 0 && (
        <div className="text-center py-8 border-2 border-dashed border-muted-foreground/25 rounded-lg">
          ...
          <p className="text-sm text-muted-foreground">
            Agrega hasta {maxImages} imágenes para mostrar el comercio
          </p>
        </div>
      )}
    </div>
