...
const formSchema = z.object({
  specialty: z.string().min(1, {
    message: "La Especidalidad es requerida",
  }),
});

export const SpecialtyInput = ({ id, initialData }: SpecialtyInputProps) => {
  const { toast } = useToast();
  const [isEditing, setisEditing] = useState(false);
  const [currentData, setCurrentData] = useState(initialData);

  const specialty = currentData?.specialty || "";
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      const updatedContact = await setProfessionalInfo(id, values);
        ...
        <div className="pt-6 sm:flex">
          ...
          <dd className="mt-1 flex justify-between items-center gap-x-6 sm:mt-0 sm:flex-auto">
            {isEditing ? (
              ...
              <div className="text-gray-900">{specialty}</div>
            )}
